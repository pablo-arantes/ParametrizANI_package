...
class DihedralOptimizer:
    ...
    def run_optimization(
        ...
        for run_idx, sequence in enumerate(self.run_sequences[:self.max_terms]):
            ...
            for j, (coeff, mult, phase) in enumerate(zip(coeffs, sel_mult, sel_phase)):
                params[j + 1] = {'V_n': float(coeff), 'phase': float(phase), 'periodicity': int(mult)}
        ...
        frcmod_params = self._format_frcmod_params(best_params, atom_types)
    ...
    def _format_frcmod_params(self, params, atom_types=None, idivf=1):
        ...
        if atom_types is None:
            atom_types = ['X', 'X', 'X', 'X']
        type_str = f"{atom_types[0]:>2s}-{atom_types[1]:>2s}-{atom_types[2]:>2s}-{atom_types[3]:>2s}"
        lines = ["DIHE"]
        for term_n, p in params.items():
            v_n = p['V_n']
            phase = p['phase']
            periodicity = p['periodicity']
            pn_sign = -periodicity if term_n < len(params) else periodicity
            line = f"{type_str}   {idivf}   {v_n:10.4f}  {phase:8.2f}  {pn_sign:6.1f}"
            lines.append(line)
        return "\n".join(lines)
    
    def write_frcmod(self, result, output_file=None, idivf=1):
        """Write optimized parameters to FRCMOD file."""
        if output_file is None:
            output_file = os.path.join(self.work_dir, 'optimized.frcmod')
        frcmod_content = self._format_frcmod_params(
            result['best_parameters'], result.get('atom_types'), idivf
        )
